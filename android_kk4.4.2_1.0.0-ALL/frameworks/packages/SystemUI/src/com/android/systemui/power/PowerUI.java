...
import android.iline.ILineLight;
...
public class PowerUI extends SystemUI {
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
				...
				/* add by lianghao 2017.9.29 Turn on the light according to the size of the charge bagain*/
				if (mBatteryLevel <= 10) {
					Slog.d(TAG, "now mBatteryLevel<=:" + 10);			
					mLight.setLight(ILineLight.GREEN,0);
					mLight.setLight(ILineLight.YELLOW,0);
					mLight.setLight(ILineLight.RED,1);
				} else if (mBatteryLevel >= 20) {
					Slog.d(TAG, "now mBatteryLevel>=:" + 20);
					mLight.setLight(ILineLight.RED,0);					
					mLight.setLight(ILineLight.YELLOW,0);				
					mLight.setLight(ILineLight.GREEN,1);
				} else {
					Slog.d(TAG, "now mBatteryLevel<=>10|20:");
					mLight.setLight(ILineLight.GREEN,0);
					mLight.setLight(ILineLight.RED,0);
					mLight.setLight(ILineLight.YELLOW,1);
				}
				/* add by lianghao 2017.9.29 Turn on the light according to the size of the charge end*/
	...
	/*
	 * modify by lvyanming for system lowBatterywarning dialog style 2017-10-18
	 * begin
	 */
	void showLowBatteryWarning() {
		Slog.i(TAG, ((mBatteryLevelTextView == null) ? "showing" : "updating")
		...
		if (mBatteryLevelTextView != null) {
			...
			b.setView(v);
			Button button_ok = (Button) v.findViewById(R.id.button_ok_id);
