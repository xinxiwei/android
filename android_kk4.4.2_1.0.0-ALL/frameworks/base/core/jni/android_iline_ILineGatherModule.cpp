...
pthread_t th_RcvData;
 ...
 jmethodID method_request_java_cb1 = NULL;
...
static void request_java_callback(float data[],jboolean isCollectData){
    for(int i= 0;i<1;i++){
		ALOGE("jnitest: Gather_data[%d] = %f",i,data[i]);
	}
...
static void request_press_stop_callback(jboolean isStop){
	...
	JNIEnv* env = AndroidRuntime::getJNIEnv();
	env->CallVoidMethod(mCallbacksObj, method_request_java_cb1, isStop);
	
	gJavaVm = NULL;
	env->DeleteGlobalRef(mCallbacksObj);
	method_request_java_cb1 = NULL;
	method_request_java_cb = NULL;
	mCallbacksObj = NULL;
};
...
static void android_debug_JNITest_startSingleAD(JNIEnv* env, jobject object,float data){  
	...
	method_request_java_cb1 = env->GetMethodID(clazz,"requestStopPress", "(Z)V");
