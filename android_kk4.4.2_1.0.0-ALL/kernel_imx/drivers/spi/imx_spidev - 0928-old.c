...
static int spidev_alloc_mem_rxframe(struct spi_imx_data *spi_devdata )
{
	...
	for (i = 0; i < spi_devdata->num_rxdma_bufs; i++) {
		...
		printk("paddress = 0x%x ,vaddress = 0x%x",spi_devdata->rxsegnums[i].paddress,spi_devdata->rxsegnums[i].vaddress);
...
{ 
	...
	sg = &frame->rx_sgl;
    printk("2222paddress = 0x%x ,vaddress = 0x%x",frame->paddress,frame->vaddress);
