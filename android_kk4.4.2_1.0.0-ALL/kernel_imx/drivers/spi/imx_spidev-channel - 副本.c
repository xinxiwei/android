...
static struct dma_chan chan_test ;
...
static int spi_imx_dma_init(struct spi_imx_data	*spi_imx)
{
    ...
    memcpy(&chan_test,spi_imx->dma_chan_rx,sizeof(struct dma_chan));	
...
static int spidev_preparation_work(struct spi_imx_data *spi_imx)
{
	struct spi_imx_config config;
	
	spi_imx_dma_init(spi_imx); 
	config.bpw = 32;
...
{ 
	...
	if( enable_num >= 1  ){
        ...
        if(spi_imx == NULL)
			return -1;
        //spi_imx->dma_chan_rx = &chan_test ;
	    if(spi_imx->dma_chan_rx == NULL)
		   printk("spidev_rxstreamon: dma_chan_rx \n");
		//memcpy(spi_imx->dma_chan_rx,&chan_test,sizeof(struct dma_chan));
		printk("spidev_rxstreamon: 222222222222 \n");
	}
    printk("spidev_rxstreamon: 33333333333 \n");
...
static int spidev_imx_open(struct inode *inode, struct file *filp)
{
	...
	if (ret != 0) {
	  ...
	  return -ENODEV;
	   ...
	   //spidev_preparation_work(spi_devdata);  	       
       mutex_unlock(&device_list_lock);      
...
static long spidev_imx_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
  ...
  switch (cmd) {        
	...
	case SPIDEV_IOC_RXSTREAMON: {
		...
		//spidev_preparation_work(spi_devdata);
		ret = spidev_rxstreamon(spi_devdata);
