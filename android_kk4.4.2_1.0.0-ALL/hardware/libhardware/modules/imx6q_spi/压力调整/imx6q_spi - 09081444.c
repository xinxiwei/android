...
{	
	...
	LOGD("StopSampleAddr = %d, StopSampleData = %d",StopSampleAddr,StopSampleData);
...
static int stop_press_sample( struct spictl_device_t* dev )//停止压力采集
{	
	LOGD( "xin：停止压力采集开始 power_off_flag = %d, restart_power_on_flag = %d,start_enable_flag =%d is : [%s]\n" , power_off_flag, restart_power_on_flag, start_enable_flag, log_time( ) );
	#if 1
	...
	LOGD( "xin：停止压力采集结束 power_off_flag = %d, restart_power_on_flag = %d,start_enable_flag =%d is : [%s]\n" , power_off_flag, restart_power_on_flag, start_enable_flag, log_time( ) );
	return 0;	
}
...
{	    	
	...
	{
		value1[i] = value1[2*i];
		value2[i] = value2[2*i+1];
						
	}	
    for(i =0 ;i<= length/2;i++)
	{
		if( fabs(value1[i]) < EPSILON )
		{			
			num1 ++;
		}
		 if( fabs(value2[i]) < EPSILON )
		{			
			num2 ++;		
		}
	}
	LOGD("000000num1 = %d, num2 = %d", num1,num2);
	#endif
}
...
{    
	...
	{		
		...
		if(power_off_flag == 1) 
		{
			...
			pressure_callback_backup.mspictl_callback( invalid_buf , false );				
		    return NULL;
		}else{
...
{	    
    ...
    if(restart_power_on_flag == 1) // 当内部stop_DMA，fpga,关闭设备后，flag 置1， 当再启动start接口时不再重新上电，当对外大的停止采集下电后，此flag会置为 0，重新采集时再上电
	{
		...
		if( reg_ret_value == -1)
		{			
			pressure_callback_backup.mspictl_callback( reg_fail_buf ,  false );//表示寄存器配置失败，回调 false给上层
			start_enable_flag = 0;
            spi_poweroff(  );			
			return 0;
		}
...
void *total_rend_thread( void* arg ) //总值趋势线程
{
	...
	{		        
		...
		LOGD( "xin: SINGLE_CH_结束转换数据 is : [%s]\n" ,  log_time( ) );
		    ...
		    for(i =0;i< 50; i++)
			{
				LOGD("rend_CH1_smp_buf[%d] = %f",i,rend_CH1_smp_buf[i]);
			}		
		#endif
