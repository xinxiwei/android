...
{    
	...
	{		
		...
		if(power_off_flag == 1) 
		{
			LOGD("xin: 数据线程中检测到下电标识power_off_flag ==1 开始回调false 给上层");
			can_start_flag = 1;
			spi_power_off(); 
			sem_destroy( &run_sem );
...
void *total_rend_thread( void* arg ) //总值趋势线程
{
	...
	{		        
		...
		{	  
			...
			//{
				//LOGD("first_data_flag =0 表示第一波数据异常，需要丢弃");
				//vibrate_callback_backup.single_ch_callback( NULL , 1 ,  true ); /////回调
			//}else{
