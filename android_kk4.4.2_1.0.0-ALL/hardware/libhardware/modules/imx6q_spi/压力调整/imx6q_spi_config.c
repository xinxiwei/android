...
int set_singleCH_vibrate_reg( int signalType , float maxFreq ,  float minFreq ) //设置振动采集寄存器，单通道振动默认CHB
{
    ...
    //set_24V( CHB ,  true );//设置24v电源的开关激励 , 默认不开启
...
int set_doubleCH_vibrate_reg( int signalType , float maxFreq ,  float minFreq )//设置振动采集寄存器
{
	...
	{		
		LOGD( "xin:=== signalType == 0 ,  ACC" );
		error_value1 = enable_CH_A( true );//使能CHA
		error_value2 = enable_CH_B( true );//使能CHA
		if( minFreq < 7 )  //小于7Hz以下建议用DC耦合，大于等于7HZ 用AC耦合
		{
			//LOGD( "xin:=== minFreq <7HZ ,  DC" ); 
			set_couple_mode( CHA ,  DC );//耦合方式 DC
			set_couple_mode( CHB ,  DC );//耦合方式 DC
		}else{
			//LOGD( "xin:=== minFreq >7HZ ,  AC" );
			//set_couple_mode( CHA ,  AC );//耦合方式 ,  默认为AC
			//set_couple_mode( CHB ,  AC );//耦合方式 ,  默认为AC
		}
	}else if( signalType == 1 ) //速度
	{
		LOGD( "xin:=== signalType == 1 ,  SPEED" );
		enable_CH_A_integrate( true );//使能CHA的积分
		enable_CH_B_integrate( true );//使能CHA的积分
	...
	//set_24V( CHA ,  true );//设置24v电源的开关激励 , 默认不开启
	//set_24V( CHB ,  true );//设置24v电源的开关激励 , 默认不开启
	...
	//---------------
	//LOGD( "set_doubleCH_vibrate_reg error_value = %d" , error_value );
...
int set_rotation_reg(  )//设置转速采集寄存器
{
	...
	enable_rotation( true );//使能转速通道
	set_speed_resistance( para.rotation_resis_value );//设置转速分压电阻值
	set_speed_triger_mode( AnalogFallingCount );//设置转速触发方式  , AnalogFallingCount , AnalogRisingCount
	//---------------
	//LOGD( "set_rotation_reg error_value = %d" , error_value );
