...
volatile bool stop_press_smp_flag = false; // 点击stop时的停止压力采集标识
volatile bool stop_pread_flag = false; //压力停止读数据标识
...
void common_start()//打开设备和开始fpga采集
{
    ...
    if(stop_press_smp_flag) // 停止压力采集标识
	{
    ...
    usleep(DELAY_300000); //用于 stop_press_smp_ flag 和 fd 的在停止时的变化能够生效，防止后面多一次打开DMA, 重要！！！！
    if(stop_press_smp_flag) // 停止压力采集标识
    {
...
static int stop_press_sample(struct spictl_device_t* dev)//停止压力采集
{
    ...
    stop_press_smp_flag = true; 
	if(!start_enable_flag)
	{
		LOGD("停止压力采集, 此时start线程刚结束，无线程存在, 直接回调false给上层"); 
		usleep(DELAY_500000); //用于等待stop_press_smp_ flag 变量变化，防止在还没有关完前，重新响应start
        if(fd != -1)
        {
            stop_fpga_dma(); 
            poweroff_spi();  //下电
...
{
    LOGD("读取压力数据时stop_pread_ flag = %d, p_loop_num = %d, stop_press_smp_ flag =%d, [%s]\n", stop_pread_flag, p_loop_num, stop_press_smp_flag, log_time()); 
    ...
    if(shang > 0 && yu > 0)
    {
        ...
        }else if(p_loop_num == shang)
        {
            if(press_smp_discard_flag >= 1) //表示丢前一波数据
            {
                stop_pread_flag = true; // flag置1 ，停止继续读数据
...
void *press_curve_thread(void* arg) //压力曲线模式
{
    ...
    float c_bar_err_flag[3] ={10001, 10001, 10001}; 	//压力异常回调时的状态数组 ， 1表示数据有效个数为1
	float value_bar_max2 = 0.0, value_bar_min2 = 0.0; 
	float tmp1 = 0.0, tmp2 = 0.0, tmp3 = 0.0; 

    if(stop_press_smp_flag)
    {
	...
	if(tmp3  <= 0.01)  //0.5bar = 0.01V  // 20mv/bar  表示外部环境和气缸环境 相差很小，此时表示空采
	{
		...
		pressure_callback_backup.mspictl_callback(c_bar_err_flag, false); //小于0.5bar
		return NULL; 
	}
	else if(tmp3 > 0.01)
	{
		...
		if(stop_press_smp_flag)//检测到停止压力采集标识为1
		{
...
void *press_dial_thread(void* arg) //压力表盘模式
{
    ...
    float d_bar_err_flag[3] ={10001, 10001, 10001}; 	//压力异常回调时的状态数组 ， 1表示数据有效个数为1
	float final_buf[SIZE_PRESS] ={0.0}; 
	float positive_buf[SIZE_PRESS] ={0.0}; 

	float value_bar_max1 = 0.0, value_bar_min1 = 0.0; 
	float f64_sum = 0.0, tmp1=0.0, tmp2=0.0, tmp3=0.0; 
	...
	if(tmp3 <= 0.01)  //0.5bar = 0.5*20mv = 10mv = 0.01V  //外部环境和气缸环境 相差很小，此时表示空采
	{
		...
		pressure_callback_backup.mspictl_callback(d_bar_err_flag, false); //小于0.5bar
		return NULL; 
	}
	else if(tmp3 > 0.01)
	{
...
static int start_pressure_curve(struct spictl_device_t* dev, float flag0_value)  //压力曲线模式
{
    ...
    if(restart_power_on_flag) // 当内部stop DMA，fpga, 关闭设备后，flag 置1， 当再启动start接口时不再重新上电，当对外大的停止采集下电后，此flag会置为 0，重新采集时再上电
	{
        ...
        usleep(DELAY_3000000); //用于每次上电后，上层时域波形不丢波形数据，在这里直接延时3S, 让硬 件预热稳定
	}
