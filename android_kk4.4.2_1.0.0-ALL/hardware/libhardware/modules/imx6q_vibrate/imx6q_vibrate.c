...
inline float adc_data_to_24value(int adc_data)// 将采集32位数据 转换为24位有效数据
{
    ...
    value = value *10; //乘以10 是因为寄存器默认配置是25V，硬件电路会衰减10倍，再此*10补上, 和24V激励没有关系
...
void *time_wave_thread(void* arg) //时域线程
{
    ...
    LOGD("时域线程中 g_chNum = %d, t_discard_pnts = %d, t_max_freq = %d, t_min_freq = %d, t_wave_length = %d", g_chNum, t_discard_pnts, t_max_freq, t_min_freq, t_wave_length);
...
static int start_vibrate_CH_timewave(struct spictl_device_t* dev, int ch_num, struct time_wave_para tWave)//时域波形
{
	...
	t_discard_pnts = get_invalid_num(t_max_freq, t_min_freq);

    LOGD("响应时域开始signal_type = %d, min_freq = %d, max_freq = %d, wave_length = %d, tWave.version_mode = %d", tWave.signal_type, t_min_freq, t_max_freq, t_wave_length, tWave.version_mode);

	if(!is_valid_length(t_wave_length)) //判断下发的采样长度是否支持
	{
    ...
    if(restart_power_on_flag == 1) // 当内部stop DMA，fpga, 关闭设备后，flag 置1， 当再启动start接口时不再重新上电，当对外大的停止采集下电后，此flag会置为 0，重新采集时再上电
	{
		...
		if(g_chNum == SINGLE_CH)
        {
			reg_ret_value = set_singleCH_vibrate_reg( tWave.signal_type , t_max_freq , t_min_freq, tWave.version_mode);//设置单通道采集寄存器
        }
		if(reg_ret_value == -1)
		{
...
static int start_vibrate_CH_totalrend(struct spictl_device_t* dev, int ch_num, struct total_rend_para tRend)//总值趋势
{
	...
	r_discard_pnts = get_invalid_num(r_max_freq, r_min_freq);

    LOGD("响应总值开始signal_type = %d, min_freq = %d, max_freq = %d, wave_length = %d, tRend.version_mode = %d", tRend.signal_type, r_min_freq, r_max_freq, tRend.wave_length, tRend.version_mode);
	...
	if(restart_power_on_flag == 1) // 当内部stop DMA，fpga, 关闭设备后，flag 置1， 当再启动start接口时不再重新上电，当对外大的停止采集下电后，此flag会置为 0，重新采集时再上电
	{
		...
		if(g_chNum == SINGLE_CH)
		{
            reg_ret_value = set_singleCH_vibrate_reg( tRend.signal_type , r_max_freq , r_min_freq, tRend.version_mode);//设置单通道采集寄存器
		}
		if(reg_ret_value == -1)
		{
            LOGD("寄存器配置失败, 回调寄存器失败数据10001, flag全部初始化为0");
...
static int start_vibrate_evalute_level(struct spictl_device_t* dev, struct time_wave_para tWave)//振动等级评估
{
    ...
    e_discard_pnts = get_invalid_num(e_max_freq, e_min_freq);

	LOGD("响应振动等级评估开始signal_type = %d, min_freq = %d, max_freq = %d, wave_length = %d, tWave.version_mode = %d", tWave.signal_type, e_min_freq, e_max_freq, tWave.wave_length, tWave.version_mode);
	...
	poweron_spi(  );
	int reg_ret_value = set_singleCH_vibrate_reg( tWave.signal_type , e_max_freq , e_min_freq, tWave.version_mode );//设置单通道采集寄存器，数据类型为 速度时域波形
	if(reg_ret_value == -1)
	{
		LOGD("寄存器配置失败, 回调寄存器失败数据10001, flag全部初始化为0");
		flag_init(); //停止和异常回调 对所有标识flag清0
