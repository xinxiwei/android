...
unsigned char max_char_buf[VIB_MAX_SIZE] = {0};
...
void close_init() //回调前统一运行函数
{
    LOGD("振动close_init====，下电，关闭主设备，清flag");
    poweroff_spi();//下电
	masterspi_close();//关闭主设备
    flag_init();//对所有标识flag 清0
}
...
{    
    //LOGD("stop_fpga-dma停止FPGA, DMA , fd = %d, is : [%s]\n", fd, log_time());
...
static int stop_vibrate_sample(struct spictl_device_t* dev)//停止振动采集，会将stop_vibrate_smp_ flag 置为1，若为1，stop_fpga-dma时会下电
{
    ...
    LOGD("停止振动采集stop_vibrate_smp_flag = 1，post信号量");
    sem_post(&run_sem);
	return 0;
}
...
void *time_wave_thread(void* arg) //时域线程
{
	...
	{        
        ...
        if(VOL_RANGE_V25 == vol_range)//表示电压量程是25V
        {
            for (i= 0; i< smp_length; i++)  
            {
                analyze_CH_data(smp_buf[i], t_CH_data);
                time_CH1_smp_buf[i] = t_CH_data[2] * 10; //乘以10 是因为寄存器默认配置是25V，硬件电路会衰减10倍，再此*10补上, 和24V激励没有关系
            }
        }else if(VOL_RANGE_V0P25 == vol_range)//表示电压量程是0.25V
        {
            for (i= 0; i< smp_length; i++)  
            {
                analyze_CH_data(smp_buf[i], t_CH_data);
                time_CH1_smp_buf[i] = t_CH_data[2] / 10; //除以10 是因为硬件电路增益放大了10倍，再此除以10 去掉增益
            }
        }
		        
    	if(auto_voltagerange_flag)
    	{  
        	t_pkvalue = rend_value(time_CH1_smp_buf, t_wave_length, SG_TIMESIGNAL_HANDLE_PK);	//求峰值
        	goto  auto_volrange;  
    	}
        ...
        if(fabs(t_calib_para[0]) < EPSINON)
        {
            t_calib_para[0] = 1.0;
        }
        ...
        auto_volrange:
        {
	        if(stop_vibrate_smp_flag)
			{
                ...
                else //回调正常数据
                {
                    ...
                    if(stop_vibrate_smp_flag)
                    {
                        LOGD("时域回调正常数据时，检测到手动停止标识");
                        flag_init();//回调前 对所有标识flag清0
                        vibrate_callback_backup.single_ch_callback(t_invalid_buf, 0, false);//用于停止采集时的异常回调
                    }else{
                        LOGD("时域回调正常数据");
                        flag_init();//回调前 对所有标识flag清0
...
void *total_rend_thread(void* arg) //总值趋势线程
{
	...
	if(my_totalrend.chan_num == CH_B)
	{
       ...
       // LOGD("总值趋势线程 signal_type = %d, max_freq = %d, min_freq = %d, vol_range = %d, auto_voltagerange_flag = %d", my_totalrend.signal_type, r_max_freq, r_min_freq, vol_range, auto_voltagerange_flag);
        if(VOL_RANGE_V25 == vol_range)
        {
            for (i= 0; i< smp_length; i++)  
            {
                analyze_CH_data(smp_buf[i], r_CH_data);
                rend_CH1_smp_buf[i] = r_CH_data[2] * 10; 
            }
        }else if(VOL_RANGE_V0P25 == vol_range)
        {
            for (i= 0; i< smp_length; i++)  
            {
                analyze_CH_data(smp_buf[i], r_CH_data);
        ...
        if(auto_voltagerange_flag) 
        {  
            r_pkvalue = rend_value(rend_CH1_smp_buf, r_wave_length, SG_TIMESIGNAL_HANDLE_PK);	//求峰值 
            goto  auto_volrange;  
        }
    #if READ_CALIB_PARA
        LOGD("从FRAM 读取校准参数===");
        read_calib_para(my_totalrend.signal_type, r_max_freq, r_min_freq, vol_range, r_calib_para);  //读取校准参数 
        if(fabs(r_calib_para[0])< EPSINON)
        {
            r_calib_para[0] = 1.0;
        }
        ...
        dis_dc_func(rend_CH1_smp_buf, smp_length); 
    ...
    if(my_timewave.chan_num == CH_B)
    {
        ...
        if(VOL_RANGE_V25 == vol_range)
        {
            for (i= 0; i< smp_length; i++)
            {
                ...
                evalute_CH1_smp_buf[i] = e_CH_data[2] * 10; 
            }
        }else if(VOL_RANGE_V0P25 == vol_range)
        {
            for (i = 0; i< smp_length; i++)
        ...
        if(fabs(e_calib_para[0]) < EPSINON)
        {
            e_calib_para[0] = 1.0;
        }   
        for (i= 0; i< smp_length; i++)
		...
		evalute_value[0] = rend_value(evalute_CH1_smp_buf, e_wave_length, SG_TIMESIGNAL_HANDLE_RMS);	//速度有效值 0表示有效值
...
static int start_vibrate_CH_timewave(struct spictl_device_t* dev, struct time_wave_para tWave)//时域波形
{
    ...
    memset(max_char_buf, 0, VIB_MAX_SIZE * sizeof(unsigned char));
    ...
    if(!auto_voltagerange_flag)
    {
		...
		if((ACC_TYPE == tWave.signal_type && (int)tWave.min_freq < FREQ_7) 
            || ((VEL_TYPE == tWave.signal_type || DSP_TYPE == tWave.signal_type) && (int)tWave.min_freq < FREQ_10)) //DC模式时，重新将电压量程设为25V
        {
...
static int start_vibrate_CH_totalrend(struct spictl_device_t* dev, struct total_rend_para tRend)//总值趋势
{
     ...
     if(!auto_voltagerange_flag)
    {
		...
		if((ACC_TYPE == tRend.signal_type && (int)tRend.min_freq < FREQ_7) 
            || ((VEL_TYPE == tRend.signal_type || DSP_TYPE == tRend.signal_type) && (int)tRend.min_freq < FREQ_10)) //DC模式时，重新将电压量程设为25V
        {
            LOGD("DC模式强制切换到 25V量程===");
            vol_range = VOL_RANGE_V25;
        }
