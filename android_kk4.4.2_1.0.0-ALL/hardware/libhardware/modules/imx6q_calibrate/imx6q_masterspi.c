...
#define DEVICE_NAME "/dev/spidev0.0"  //主设备, 用于下发指令，读写FPGA内部寄存器
/*****************************************************************************/
...
int swrite(unsigned int  laddr, unsigned int data)
{
	...
	//{
		LOGD("write addr = 0x%x, value = %d", (localbuf[0]&0x00ffffff), localbuf[1]);
	//}
	usleep(20000); //50000
...
unsigned int sread(unsigned int addr)
{
	...
	//if(masterspi_testcmd == 1 && masterspi_testpara == 1)
	//{
		LOGD("read addr = 0x%x, value = %d", (tbuf[0]&0x00ffffff), localbuf[0]);
	//}
	return  localbuf[0];
}
