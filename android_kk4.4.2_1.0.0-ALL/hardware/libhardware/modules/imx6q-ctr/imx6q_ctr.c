...
#define DEVICE_NAME "/dev/devpwerctr"
#define MODULE_NAME "devpwerctr"
...
static int gpio_set_val(struct gpioctl_device_t* dev,int value1,int value2);
static int gpio_get_val(struct gpioctl_device_t* dev,int* val);
/*模块方法表*/
...
static int gpio_set_val(struct gpioctl_device_t* dev,int value1,int value2){
	...
	ALOGE("gpioctr gpiofd = %d.",gpiofd);
     if(gpiofd == -1){		
        ALOGE("gpioctr Stub: failed to open /dev/devpwerctr -- %s.",strerror(errno));
        free(dev);
        return -EFAULT;
    }else{
		ALOGD( "gpioctr Stub: open /dev/devpwerctr successfully.");
	}
