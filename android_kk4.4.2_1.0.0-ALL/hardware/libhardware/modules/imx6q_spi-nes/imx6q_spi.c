...
void *press_flag0_thread( void* arg ) //压力标0模式
{
	...
	{		        
		...
		f64_sum += adc_data_to_24value( psample_buf[i] ) *10 ;	
	}	
...
{    
	...
	for( i=0;i<SIZE_16K;i++ )
	{
		...
		final_buf[i] = adc_data_to_24value( psample_buf[i] )*10;	           		
...
{	
   ...
   if(start_num != 0)
   {
    ...
    LOGD("xin: 点击时域开始时 start_enable_flag = %d, restart_power_on_flag= %d, can_start_flag = %d , thread_finished_flag = %d",start_enable_flag,restart_power_on_flag,can_start_flag,thread_finished_flag);
